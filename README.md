...
결혼식 · 장례식 같은 경조사의 **명부와 경조사금**을 행사별로 정리하고, 엑셀로 내보내는 Windows 데스크톱 프로그램입니다.
...
행사 하나를 고르면 오른쪽에 그 행사의 명부와 집계가 나옵니다.
...
명부가 수만 명이면 엑셀 내보내기에 시간이 걸립니다. 그동안 본문이 흐려지고 가운데에서 동그라미가 돕니다. 제목줄은 흐려지지 않아 그대로 창을 옮기거나 닫을 수 있습니다.

![진행 표시](docs/screenshot-busy.png)

**식권을 쓰지 않는 행사**(예: 장례식)에서는 식권 열 · 식권합계 열 · 식권 타일 · 식대 차감 타일이 화면과 엑셀에서 모두 사라집니다. 장례식에서는 "하객"이 축하하러 온 손님이라는 뜻이라 쓸 수 없으므로 표 제목과 버튼도 **조문객**으로 바뀝니다. 위 결혼식 화면과 비교해 보세요.
...
| 명부 | 표에서 바로 입력. 이름 · 금액 · 관계 · 소속 · 식권 · 비고 |
| 호칭 구분 | 행사 종류에 맞춰 **하객** / **조문객**으로 표시. 표 제목 · 버튼 · 엑셀 파일명까지 따라갑니다 |
